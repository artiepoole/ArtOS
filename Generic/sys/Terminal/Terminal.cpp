...
void Terminal::_putChar(const terminal_char_t ch, const u32 origin_x, const u32 origin_y) {
    ...
    if (origin_x + scaled_char_dim < screen_region.x2 && origin_y + scaled_char_dim < screen_region.y2) {
        ...
        const size_t line_step_amount = screen_region.w - scaled_char_dim;
        // amount to step to start next lin of character
...
void Terminal::setScale(u32 new_scale) {
    ...
    if (new_scale * char_dim < screen_region.w && new_scale * char_dim < screen_region.h) {
        ...
        terminal_buffer = static_cast<terminal_char_t *>(art_alloc(
            buffer_height * buffer_width * sizeof(terminal_char_t), 0));
        rendered_buffer = static_cast<terminal_char_t *>(art_alloc(
            buffer_height * buffer_width * sizeof(terminal_char_t), 0));
...
void Terminal::setRegion(const u32 x, const u32 y, const u32 width, const u32 height) {
    ...
    buffer_width = screen_region.w / scaled_char_dim;
    buffer_height = screen_region.h / scaled_char_dim;
    terminal_buffer = static_cast<terminal_char_t *>(art_alloc(buffer_height * buffer_width * sizeof(terminal_char_t),
                                                               0));
    rendered_buffer = static_cast<terminal_char_t *>(art_alloc(buffer_height * buffer_width * sizeof(terminal_char_t),
                                                               0));
    term_screen_buffer = static_cast<u32 *>(art_alloc(screen_region.h * screen_region.w * sizeof(u32), 0));
    ...
    terminal_row = 0;
    terminal_column = 1;
    VideoGraphicsArray::get().fillRectangle(screen_region.x1, screen_region.y1, screen_region.w, screen_region.h,
                                            colour_bkgd);
    owns_screen = true;
}
...
void Terminal::_draw_changes() {
    ...
    _update_cursor();
    size_t i = 0;
    size_t min_row = buffer_height - 1;
    size_t max_row = 0;
    for (size_t row = 0; row < buffer_height; row++) {
        bool draw_line = false;
        for (size_t col = 0; col < buffer_width; col++) {
            if (terminal_char_t c_to_draw = terminal_buffer[i];
                *reinterpret_cast<u64 *>(&rendered_buffer[i]) != *reinterpret_cast<u64 *>(&c_to_draw)) {
                draw_line = true;
        ...
        if (draw_line) {
            ...
            max_row = row; // No need for if because this is always increasing
        }
    }
    const auto &vga = VideoGraphicsArray::get();
    size_t start_y = min_row * scaled_char_dim;
    size_t height = scaled_char_dim * (max_row - min_row + 1);
    size_t start_idx = min_row * screen_region.w * scaled_char_dim;
    vga.copy_region(&term_screen_buffer[start_idx], screen_region.x1, start_y+screen_region.y1, screen_region.w, height);
    vga.draw();
}
