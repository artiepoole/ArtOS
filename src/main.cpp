...
void kernel_main(unsigned long magic, unsigned long boot_info_addr)
{
    ...
    u32 data_start_lba = 16; // in LBA
    CD_ROM.read_lba(&volume_descriptor, data_start_lba, buf_size);
    ...
    CD_ROM.read_lba(path_table_data, volume_descriptor.path_l_table_loc_lba , volume_descriptor.path_table_size_LE);
    // Count entries
    size_t offset = 0;
    size_t n_dirs = 0;
    ...
    iso_path_table_entry path_table[0];
    ...
    CD_ROM.read_lba(&first_sector_data, path_table[0].header.extent_loc, 2048);
    char* root_dir_name = strdup("/");
    // todo: write a convert function
    auto dir_record = *reinterpret_cast<iso_directory_record_header*>(&first_sector_data[0]);

    auto root_dir = new ArtDirectory{
        nullptr,
        DirectoryData{
            &CD_ROM,
            dir_record.extent_loc_LE,
            dir_record.data_length_LE,
            tm{
                dir_record.datetime.second,
                dir_record.datetime.minute,
                dir_record.datetime.hour,
                dir_record.datetime.monthday,
                dir_record.datetime.month,
                dir_record.datetime.years_since_1900 + 1900,
                0,
                0,
                0
            },
    ...
    size_t full_size = dir_record.data_length_LE;
    ...
    offset = dir_record.record_length;
    char full_data[full_size];
    CD_ROM.read_lba(&full_data, dir_record.extent_loc_LE, full_size);
    ...
    while (offset < full_size)
    {
        ...
        LOG("Directory: ", root_dir->get_name(), " name: ", filename, " data length: ", data_len, " flags raw: ", flags, " is directory: ", bool(flags & 0x02));
        // todo: write as func
